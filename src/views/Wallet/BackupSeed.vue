<template>
    <ion-page>
        ...
        <ion-content class="ion-padding" :fullscreen="true">
            <ion-text>
                <p>To make sure that you have written down your seed, we are going to ask you to enter 3 words of the seed. Please enter the {{ ordinal(firstWordNumber) }}, {{ ordinal(secondWordNumber) }} and {{ ordinal(thirdWordNumber) }} word of the seed phrase in order to continue.</p>
            </ion-text>

            <!-- Seed phrase word entry -->
            <ion-item-group>
                <ion-item>
                    <ion-label position="stacked">{{ ordinal(firstWordNumber) }} word</ion-label>
                    <ion-input v-model="firstWord" type="text"></ion-input>
                </ion-item>
                <ion-item>
                    <ion-label position="stacked">{{ ordinal(secondWordNumber) }} word</ion-label>
                    <ion-input v-model="secondWord" type="text"></ion-input>
                </ion-item>
                <ion-item>
                    <ion-label position="stacked">{{ ordinal(thirdWordNumber) }} word</ion-label>
